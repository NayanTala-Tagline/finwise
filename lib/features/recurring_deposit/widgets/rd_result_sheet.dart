...
class _PaymentBreakdownCard extends StatelessWidget {
  const _PaymentBreakdownCard({required this.totalInvested, required this.interest});
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              _LegendItem(color: primary, label: 'Investment', value: _fmtAmt(totalInvested)),
...
class _DonutPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final investSweep = (invested / total) * 2 * math.pi;
  ...
  @override
  bool shouldRepaint(_DonutPainter o) => o.invested != invested || o.interest != interest;
}
...
class _GrowthTimelineCard extends StatelessWidget {
  ...
  List<double> get _values => List.generate(_pts, (i) {
        ...
        final deposited = result.monthlyDeposit * m;
        final interest = result.monthlyDeposit * m * (m + 1) / 2 * result.annualRate / (12 * 100);
          ...
          Row(
            children: [
              Container(
                ...
                decoration: BoxDecoration(color: primary.withValues(alpha: 0.1), shape: BoxShape.circle),
                child: Center(
                  child: Assets.homeIcons.icRdCalculator.svg(
                    width: AppSize.w16,
                    height: AppSize.h16,
                    colorFilter: ColorFilter.mode(primary, BlendMode.srcIn),
                  ),
          ...
          Padding(
            ...
            child: Row(
              ...
              children: labels
                  .map((l) => Text(
                        l,
                        style: context.textTheme.bodySmall?.copyWith(color: Colors.black38, fontSize: AppSize.sp10),
                      ))
                  .toList(),
            ),
          ),
...
class _GrowthPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < n; i++) {
      areaPath.lineTo(px(i), py(values[i]));
    }
